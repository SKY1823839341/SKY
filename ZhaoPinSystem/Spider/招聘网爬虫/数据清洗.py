...
def get_file_data(filename):
    ...
    try:
        ...
        print("",df)
        df = spark.read.csv("51job.csv", encoding="utf-8",header=False, schema=StructType(
            [StructField("序号", StringType()),
             StructField("发布时间", StringType())]))
    except:

        pass

get_file_data("51job.csv")"""
...
import openpyxl
#读取表格内容到data
data = pd.read_csv(r'33333.csv', encoding='utf-8')
...
#保存成另一个excel文件
a.to_csv(r'333333test.csv', index=False)
...
file = pd.read_csv(r'333333test.csv', encoding='utf-8')
f = pd.DataFrame(file)
